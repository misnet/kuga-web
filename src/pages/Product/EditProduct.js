...
class EditProduct extends PureComponent {
    constructor(props){
        ...
        const historyState = history.state && history.state.state!=undefined?history.state:{
            state:{
                namePath:[],
                catalogId:0
            }
        ...
        const {state:{namePath, catalogId}}  = {
            ...historyState
        }
        ...
        if(!isNaN(productId) && productId>0){
            ...
            this.props.dispatch({
                ...
                callback:(responseData)=>{
                    ...
                    if(responseData && responseData.propkeyList){
                        const {saleProps,commonProps} = this.filterProp(responseData.propkeyList);
                        this.setState({
                            saleProps
                        });
                        this.props.dispatch({
            ...
            //跳到选择品类界面
            this.props.dispatch(
                routerRedux.replace('/product/select-catalog')
            )
        }
        this.state = {
            catalogId,
    ...
    filterProp = (propkeyList)=>{
        let saleProps = [];
        let commonProps = [];
        propkeyList.length && propkeyList.map((propkey)=>{
    ...
    getInput = (prefix='prop',cprop,rules={},form,emptyOption={}) => {
        switch(cprop.formType){
            case DICT.FORM_TYPE.CHECKBOX:
                ...
                return (
                    form.getFieldDecorator(prefix+cprop.propkeyId,rules)(
                        <CheckboxGroup options={options}/>
                    )
                );
            case DICT.FORM_TYPE.TEXTAREA:
                return form.getFieldDecorator(prefix+cprop.propkeyId,rules)(<Input.TextArea  rows={4} placeholder="请输入" maxLength="200" />);
            case DICT.FORM_TYPE.TEXT:
                return form.getFieldDecorator(prefix+cprop.propkeyId,rules)(<Input placeholder="请输入" maxLength="200" />);
            case DICT.FORM_TYPE.RADIO:
                return (
                    form.getFieldDecorator(prefix+cprop.propkeyId,rules)(<Select style={{width: "100%"}} placeholder={'请选择'+cprop.propkeyName}>
                        {[
                            emptyOption.title?<Select.Option key={emptyOption.id}>{emptyOption.title}</Select.Option>:null,
                            cprop.valueList.map((opt)=>{
                                let t = [];
                                t.push(<Select.Option key={opt.id}>{opt.propvalue}</Select.Option>)
                                return t;
                            })
                        ]}
                    </Select>)
    ...
    createFormItem=(cprop)=>{
        ...
        return (
            <Col span={12} key={cprop.id}>
                <FormItem labelCol={{ span: 5 }} wrapperCol={{ span: 15 }} label={cprop.propkeyName}>
                    {this.getInput('prop',cprop,{
                        rules: [
                            {
    ...
    onSave=()=>{
        this.props.form.validateFields((error,values)=>{
            ...
            //if(!error){
               ...
               postData['mobileContent'] = this.state.contentState.toHTML();
               console.log('111',{
                ...postData,
                propList:propData,
    ...
    onAppendPropKey=()=>{
        const {form,product:{skuList,propList}} = this.props;
    ...
    createSaleProp = (sd)=>{
        ...
        return current.valueList.reduce((a,v)=>[
            ...a,...combinations.map(c=>{
                let s = {};
                //s['propvalue'+string.id]=string.propvalue;
    ...
    beforeUpload=(file)=>{
        ...
        if (!isJPG) {
          message.error('只能上传PNG/JPG/GIF图片文件');
        }
    ...
    onUploadFile=(file)=>{
        ...
        reader.onloadend = ()=>{
            const {global:{ossSetting}} = this.props;
            ossUpload({
                ossSetting,
                targetPath:'product/',
                file,
    ...
    render() {
        ...
        const {form:{getFieldDecorator},product:{skuList,propList},global:{ossSetting}} = this.props;
        ...
        const skuTableHeader = ()=> {
            return this.state.saleProps.length>0?(<Row>
                {this.state.saleProps.map(prop=>{
                    return (
                    ...
                    return <FormItem>
                        {
                            getFieldDecorator(this.createSkuFieldName(record,'price'),{
                                initialValue:record[this.createSkuFieldName(record,'price')],
                                rules:[
                                    {
                                        required:true,
                                        message:'请输入售价'
                            ...
                            })(<InputNumber style={{width:'150px'}} min={0} precision={2} placeholder="请输入" maxLength="200" onChange={(v)=>this.changeValue(record,'price',v)}/>)
                        }
                    </FormItem>
                }
                        ...
                        <FormItem>
                            {
                                getFieldDecorator(this.createSkuFieldName(record,'cost'),{
                                    initialValue:record[this.createSkuFieldName(record,'cost')],
                                    rules:[
                                        {
                                            required:false,
                                            message:'请输入成本'
                                ...
                                })(<InputNumber  style={{width:'150px'}} min={0} precision={2} placeholder="请输入" maxLength="200" onChange={(v)=>this.changeValue(record,'cost',v)}/>)
                            }
                        </FormItem>
                    )
                }
                        ...
                        <FormItem>
                            {
                                getFieldDecorator(this.createSkuFieldName(record,'originalSkuId'),{
                                    initialValue:record[this.createSkuFieldName(record,'originalSkuId')],
                                    rules:[
                                        {
                                            required:false,
                                            message:'请输入原厂SKU编号'
                                ...
                                })(<Input placeholder="请输入" maxLength="200" onChange={(e)=>this.changeValue(record,'originalSkuId',e.target.value)}/>)
                            }
                        </FormItem>
                    )
                }
            },
            {
                ...
                render:(text,record)=>{
                    return (
                        <Popconfirm
                            title="确定删除这个SKU记录?"
                            onConfirm={() => this.onRemoveSku(record.id)}
                        >
                            <a >删除</a>
                        </Popconfirm>
                    )
        ...
        return (
            <PageHeaderWrapper title={formatMessage({id:'product.add'})} >
                <Card bordered={false}>
                    ...
                    <div>
                        <Card title={"基本信息"} type={'inner'}>
                            <Row>
                                <Col span={24}>
                                    <FormItem labelCol={{ span: 4 }} wrapperCol={{ span: 20 }} label="所在类目">
                                    <Breadcrumb style={{lineHeight:"40px",height:"40px"}}>
                                    {this.state.namePath.map((name,i)=><Breadcrumb.Item key={i} >{name}</Breadcrumb.Item>)}
                                    </Breadcrumb>
                                    </FormItem>
                                </Col>
                            </Row>
                            <Row>
                                <Col span={24}>
                                    <FormItem labelCol={{ span: 4 }} wrapperCol={{ span: 20 }} label="商品名称">
                            ...
                            <Row>
                                <Col span={12}>
                                    <FormItem labelCol={{ span: 8 }} wrapperCol={{ span: 16 }} label="商品款号">
                                ...
                                <Col span={12}>
                                    <FormItem labelCol={{ span: 8 }} wrapperCol={{ span: 16}} label="厂家编码">
                        ...
                        <Row>
                            <Col span={12}>
                                <FormItem labelCol={{ span: 8 }} wrapperCol={{ span: 16 }} label="零售价">
                            ...
                            <Col span={12}>
                                <FormItem labelCol={{ span: 8 }} wrapperCol={{ span: 16 }} label="重量单位">
                        ...
                        <Row>
                            <Col span={12}>
                                <FormItem labelCol={{ span: 8 }} wrapperCol={{ span: 16 }} label="排序权重">
                                    ...
                                    })(<InputNumber placeholder="请输入" maxLength="200" />)}
                                </FormItem>
                            </Col>
                            <Col span={12}>

                                <FormItem labelCol={{ span: 8 }} wrapperCol={{ span: 16 }} label="是否上架">
                        ...
                        <Row>
                            <Col span={12}>
                                <FormItem labelCol={{ span: 8 }} wrapperCol={{ span: 16 }} label="商品卖点">
                        ...
                        <Card title={"商品属性与SKU"} type={'inner'} style={{margin:"10px 0px"}}>
                            ...
                            <Row>
                            {propList.map(cprop=>this.createFormItem(cprop))}
                            </Row>
                        </Card>
